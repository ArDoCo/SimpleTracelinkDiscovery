...
public class TraceLinkCalculator {
    ...
    private static double matchNgram(String[] ngram1, String[] ngram2, SimilarityMeasure similarityMeasure, double similarityThreshold) {
        double match = 0;
    ...
    public static TraceLink calculateTraceLink(ModelEntity modelEntity, DocumentationSection documentationSection, SimilarityMeasure similarityMeasure,
        ...
        for (String[] ngram2 : modelEntityNgrams) {
            for (String[] ngram1 : documentationSectionNgrams) {
                if (ngram1.length == ngram2.length && matchNgram(ngram1, ngram2, similarityMeasure, similarityThreshold) >= 1.0) {
                    matches += 1;
                }
            }
        }
    ...
    public static List<TraceLink> calculateTraceLinks(List<DocumentationSection> documentationSections, List<ModelEntity> modelEntities,
            double matchNumberThreshold, SimilarityMeasure similarityMeasure, double similarityThreshold) {
        List<TraceLink> traceLinks = new ArrayList<>();

        for (DocumentationSection docuSection : documentationSections) {
            for (ModelEntity modelEntity : modelEntities) {
                TraceLink traceLink = TraceLinkCalculator.calculateTraceLink(modelEntity, docuSection, similarityMeasure, similarityThreshold);
